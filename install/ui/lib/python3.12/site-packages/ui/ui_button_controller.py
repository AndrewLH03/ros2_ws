...
class UIButtonController:
    ...
    def _calculate_button_positions(self):
        ...
        self.buttons = {
            ...
            'manual_mode': {'x': self.margin, 'y': y_start + 2 * self.button_spacing, 'w': self.button_width // 2 - 5, 'h': self.button_height},
            'perception_mode': {'x': self.margin + self.button_width // 2 + 5, 'y': y_start + 2 * self.button_spacing, 'w': self.button_width // 2 - 5, 'h': self.button_height},
    ...
    def _draw_status_info(self, panel, status_data):
        ...
        mode_color = (255, 255, 0) if mode == 'manual' else (255, 255, 255)  # Yellow for manual mode
        cv2.putText(panel, f"Mode: {mode}", 
                   (self.margin, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 
                   status_font_scale, mode_color, status_thickness)
        y_pos += line_height
        
        # Show different info based on mode
        if mode == 'manual':
            self._draw_manual_servo_status(panel, status_data, y_pos, line_height, status_font_scale, status_thickness)
        else:
            self._draw_normal_status(panel, status_data, y_pos, line_height, status_font_scale, status_thickness)
            
    def _draw_manual_servo_status(self, panel, status_data, y_pos, line_height, font_scale, thickness):
        """Draw manual servo control status information."""
        # Manual mode indicator
        cv2.putText(panel, "🎮 MANUAL SERVO CONTROL", 
                   (self.margin, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 
                   font_scale, (0, 255, 255), thickness)  # Cyan
        y_pos += line_height
        
        # Finger curl ratios
        finger_curls = status_data.get('finger_curl_ratios')
        if finger_curls and len(finger_curls) == 5:
            finger_names = ['Thumb', 'Index', 'Middle', 'Ring', 'Pinky']
            
            for i, (name, curl) in enumerate(zip(finger_names, finger_curls)):
                # Color based on curl value
                if curl > 0.8:
                    color = (0, 0, 255)  # Red for very closed
                elif curl > 0.5:
                    color = (0, 165, 255)  # Orange for medium
                elif curl > 0.2:
                    color = (0, 255, 255)  # Yellow for slight curl
                else:
                    color = (0, 255, 0)  # Green for open
                    
                cv2.putText(panel, f"{name}: {curl:.2f}", 
                           (self.margin, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 
                           font_scale * 0.8, color, thickness)
                y_pos += int(line_height * 0.8)
        else:
            cv2.putText(panel, "No finger data", 
                       (self.margin, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 
                       font_scale, (128, 128, 128), thickness)
            y_pos += line_height
            
        # Manual control tips
        cv2.putText(panel, "Terminal commands:", 
                   (self.margin, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 
                   font_scale * 0.7, (200, 200, 200), thickness)
        y_pos += int(line_height * 0.8)
        
        tips = ["'open', 'closed', 'pinch'", "'t 0.5' - thumb 50%", "'help' - show all commands"]
        for tip in tips:
            cv2.putText(panel, tip, 
                       (self.margin, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 
                       font_scale * 0.6, (160, 160, 160), thickness)
            y_pos += int(line_height * 0.7)
            
    def _draw_normal_status(self, panel, status_data, y_pos, line_height, font_scale, thickness):
        """Draw normal tracking mode status information."""
        # Hand confidence
        ...
        hand_count = len(status_data['hand_poses'].poses) if status_data['hand_poses'] else 0
        ...
        body_count = len(status_data['body_poses'].poses) if status_data['body_poses'] else 0
        ...
        cv2.putText(panel, f"Status: {emerg_text}", 
                   (self.margin, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 
                   font_scale, emerg_color, thickness)
    
    def _draw_buttons(self, panel, status_data):
        """Draw all control buttons."""
        current_mode = status_data.get('mode', 'manual')
        ...
        # Individual mode buttons with active highlighting
        manual_color = (0, 255, 0) if current_mode == 'manual' else (100, 100, 100)
        perception_color = (0, 255, 0) if current_mode == 'perception' else (100, 100, 100)
        
        self._draw_button(panel, 'manual_mode', "MANUAL", manual_color)
        self._draw_button(panel, 'perception_mode', "PERCEPTION", perception_color)
    ...
    def _handle_button_press(self, button_name):
        ...
        elif button_name == 'manual_mode':
            self._set_mode('manual')
            
        elif button_name == 'perception_mode':
            self._set_mode('perception')
    ...
    def _set_mode(self, mode):
        """Set a specific control mode."""
        # Publish mode change
        msg = String()
        msg.data = mode
        self.node.mode_pub.publish(msg)
        self.node.get_logger().info(f'Mode set to: {mode}')
        
        # Update status data immediately for UI feedback
        self.node.status_data['mode'] = mode
    
    def _cycle_mode(self):
        """Cycle through available modes (kept for backward compatibility)."""
        modes = ["manual", "perception"]
        current_mode = self.node.status_data.get('mode', 'manual')
        ...
        try:
            ...
            self._set_mode('manual')  # Default to manual mode
    ...
    def handle_key_input(self, key):
        ...
        elif key == ord('1'):  # '1' for manual mode
            self._handle_button_press('manual_mode')
        elif key == ord('2'):  # '2' for perception mode
            self._handle_button_press('perception_mode')
        elif key == ord('m'):  # 'm' for cycle mode (legacy)
            self._cycle_mode()
